...
| 除法 | $\Delta\left(\dfrac{\tilde{u}}{\tilde{v}}\right) = \dfrac{\lvert\tilde{u}\rvert \Delta\tilde{v} + \lvert\tilde{v}\rvert \Delta\tilde{u}}{\lvert\tilde{v}\rvert^2}$ |

**推导（一阶泰勒展开的统一视角）**：设 $u = \tilde{u} + \varepsilon_u$，$v = \tilde{v} + \varepsilon_v$（$\lvert\varepsilon_u\rvert = \Delta\tilde{u}$，$\lvert\varepsilon_v\rvert = \Delta\tilde{v}$）。对 $f(u,v)$ 在 $(\tilde{u},\tilde{v})$ 处做一阶泰勒展开，误差按一阶项线性传播（即前面多变量公式的直接应用）：

$$\Delta f(\tilde{u},\tilde{v}) = \lvert f(u,v) - f(\tilde{u},\tilde{v}) \rvert \approx \left\lvert\frac{\partial f}{\partial u}\right\rvert \Delta\tilde{u} + \left\lvert\frac{\partial f}{\partial v}\right\rvert \Delta\tilde{v}$$

四种运算分别代入：

- **加法** $f = u+v$：$f_u = f_v = 1 \Rightarrow \Delta = \Delta\tilde{u} + \Delta\tilde{v}$；
- **减法** $f = u-v$：$f_u = 1,\ f_v = -1$，取绝对值后同样得 $\Delta\tilde{u} + \Delta\tilde{v}$——两误差同号相加，取"最坏情况"；
- **乘法** $f = uv$：$f_u = v,\ f_v = u \Rightarrow \Delta = \lvert\tilde{u}\rvert\Delta\tilde{v} + \lvert\tilde{v}\rvert\Delta\tilde{u}$。等价地直接展开：$uv - \tilde{u}\tilde{v} = \tilde{u}\varepsilon_v + \tilde{v}\varepsilon_u + \varepsilon_u\varepsilon_v$，舍去二阶小量 $\varepsilon_u\varepsilon_v$ 后取绝对值即得；
- **除法** $f = u/v$：$f_u = \dfrac{1}{v},\ f_v = -\dfrac{u}{v^2} \Rightarrow \Delta = \dfrac{\lvert\tilde{v}\rvert\Delta\tilde{u} + \lvert\tilde{u}\rvert\Delta\tilde{v}}{\lvert\tilde{v}\rvert^2}$（分母 $v\tilde{v} \approx \tilde{v}^2$，$v \approx \tilde{v}$）。
