...
> 
一般来说，如果每个关键字的值域都不大，就可以使用 [计数排序](./计数排序.md) 作为内层排序，此时的复杂度为 $O(kn+\sum\limits_{i=1}^k w_i)$，其中 $w_i$ 为第 $i$ 关键字的值域大小．如果关键字值域很大，就可以直接使用基于比较的 $O(nk\log n)$ 排序而无需使用基数排序了．
> 
### 空间复杂度
> 
