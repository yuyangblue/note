...
锦标赛排序（英文：Tournament sort），又被称为树形选择排序，是 [选择排序](./选择排序.md) 的优化版本，[堆排序](./堆排序.md) 的一种变体（均采用完全二叉树）．它在选择排序的基础上使用优先队列查找下一个该选择的元素．
...
完成一次「锦标赛」后需要将被选出的元素去除．直接将其设置为 $\infty$（这个操作类似 [堆排序](./堆排序.md)），然后再次举行「锦标赛」选出次小的元素．
