...
快速排序的工作原理是通过 [分治](./分治.md) 的方式来将一个数组排序．
...
三路快速排序（英语：3-way Radix Quicksort）是快速排序和 [基数排序](./基数排序.md) 的混合．它的算法思想基于 [荷兰国旗问题](https://en.wikipedia.org/wiki/Dutch_national_flag_problem) 的解法．
...
内省排序（英语：Introsort 或 Introspective sort）[^ref4]是快速排序和 [堆排序](./堆排序.md) 的结合，由 David Musser 于 1997 年发明．内省排序其实是对快速排序的一种优化，保证了最差时间复杂度为 $O(n\log n)$．
...
2.  寻找每组元素的中位数（因为元素个数较少，可以直接使用 [插入排序](./插入排序.md) 等算法）．
