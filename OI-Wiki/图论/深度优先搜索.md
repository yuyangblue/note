...
DFS 常常用来指代用递归函数实现的搜索，但实际上两者并不一样．有关该类搜索思想请参阅 [DFS（搜索）](../搜索/深度优先搜索.md).
...
DFS 可以使用 [栈（Stack）](../数据结构/栈.md) 为遍历中节点的暂存容器来实现；这与用 [队列（Queue）](../数据结构/队列.md) 实现的 BFS 形成高度对应．
...
以 [邻接表（Adjacency List）](./图的存储.md#邻接表) 作为图的存储方式：
...
[DFS 树](./强连通分量.md#dfs-生成树) 有很多性质，比如可以用来求 [强连通分量](./强连通分量.md)．
