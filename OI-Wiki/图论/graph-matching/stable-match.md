...
这一引理说明，偏序集 $(\mathcal S,\preceq_M)$ 和 $(\mathcal S,\preceq_W)$ 互为 [对偶](../../数学/order-theory.md#对偶)．而且，在每个偏序下，集合 $\mathcal S$ 都构成一个 [格](../../数学/order-theory.md#有向集与格)．因为 $\mathcal S$ 是有限的，这两个格一定存在最大元和最小元．这两个最值元素，分别就是前文提到的两个版本的 Gale–Shapley 算法所得到的稳定匹配．
