...
特别地，对于集合 $X$ 和它上面的偏序关系 $P$，可以建立有向无环图 $G=(X,P)$．此时，根据 [Dilworth 定理](../../数学/order-theory.md#dilworth-定理与-mirsky-定理)，图 $G$ 的最小路径覆盖的大小，就等于它的最长反链的长度，也就是偏序集 $(X,P)$ 的宽度．因此，本节实际上给出了任意偏序集的宽度的高效计算方法．
