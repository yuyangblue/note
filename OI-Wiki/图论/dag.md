...
检验它是否可以进行 [拓扑排序](./拓扑排序.md) 即可．

当然也有另外的方法，可以对图进行一遍 [DFS](../搜索/深度优先搜索.md)，在得到的 DFS 树上看看有没有连向祖先的非树边（返祖边）．如果有的话，那就有环了．

## 应用

### DP 求最长（短）路

在一般图上，求单源最长（短）路径的最优时间复杂度为 $O(nm)$（[Bellman–Ford 算法](./最短路.md#bellmanford-算法)，适用于有负权图）或 $O(m \log m)$（[Dijkstra 算法](./最短路.md#dijkstra-算法)，适用于无负权图）．
