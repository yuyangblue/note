...
Tarjan 算法是一种 **离线算法**，需要使用 [并查集](../数据结构/dsu.md) 记录某个结点的祖先结点．做法如下：
...
在 [Link Cut Tree](../数据结构/lct.md) 中，设连续两次 [access](../数据结构/lct.md#access) 操作的点分别为 `u` 和 `v`，则第二次 [access](../数据结构/lct.md#access) 操作返回的点即为 `u` 和 `v` 的 LCA.
...
注意到欧拉序满足相邻两数之差为 1 或者 -1，所以可以使用 $O(n) \sim O(1)$ 的 [加减 1RMQ](../专题/rmq.md#加减-1rmq) 来做．
