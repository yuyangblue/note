...
本文默认读者已阅读并了解了 [图论相关概念](./图论概念.md) 中的基础内容，如果在阅读中遇到困难，也可以在 [图论相关概念](./图论概念.md) 中进行查阅．
...
在 [Kruskal 算法](./最小生成树.md#kruskal-算法) 中，由于需要将边按边权排序，需要直接存边．
...
查询是否存在 $u$ 到 $v$ 的边：$O(d^+(u))$（如果事先进行了排序就可以使用 [二分查找](../算法基础/二分.md) 做到 $O(\log(d^+(u)))$）．
...
优点是边是带编号的，有时会非常有用，而且如果 `cnt` 的初始值为奇数，存双向边时 `i ^ 1` 即是 `i` 的反边（常用于 [网络流](./网络流.md)）．
