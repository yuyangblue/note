...
Tarjan 算法是一种 **离线算法**，需要使用 [并查集](../数据结构/并查集.md) 记录某个结点的祖先结点．做法如下：
...
在 [Link Cut Tree](../数据结构/动态树.md) 中，设连续两次 [access](../数据结构/动态树.md#access) 操作的点分别为 `u` 和 `v`，则第二次 [access](../数据结构/动态树.md#access) 操作返回的点即为 `u` 和 `v` 的 LCA.
