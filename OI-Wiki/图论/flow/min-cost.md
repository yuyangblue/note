...
如果使用 [Bellman–Ford 算法](../shortest-path.md#bellmanford-算法) 求解最短路，每次找增广路的时间复杂度为 $O(nm)$．设该网络的最大流为 $f$，则最坏时间复杂度为 $O(nmf)$．事实上，SSP 算法是 [伪多项式时间](../../杂项/cc-basic.md#pseudo-polynomial-time-伪多项式时间) 的．
