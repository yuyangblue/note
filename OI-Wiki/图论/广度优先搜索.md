...
下文中 C++ 与 Python 的代码实现是基于链式前向星的存图方式，其实现可参考 [图的存储](./图的存储.md) 页面．
...
如果你不了解双端队列 `deque` 的话，请参阅 [deque 相关章节](../编程语言/csl/序列式容器.md#deque)．
...
优先队列，相当于一个二叉堆，STL 中提供了 [`std::priority_queue`](../编程语言/csl/容器适配器.md)，可以方便我们使用优先队列．
...
诶？这怎么听起来这么像堆优化的 [Dijkstra](./最短路.md#dijkstra-算法) 算法呢？事实上，堆优化 Dijkstra 就是优先队列 BFS．
