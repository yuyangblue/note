...
-   **移动访问**：$w(j,i)$ 可以从 $w(j\pm 1,i)$ 或 $w(j,i\pm 1)$ 以 $O(1)$ 时间转移得到．（类似 [莫队算法](../../杂项/莫队算法.md) 中的情形）
...
这一结论保证了可以通过 WQS 二分（国外称 Aliens Trick）的方法解决此问题．具体来说，考虑带参的成本函数 $w_c(j,i):=w(j,i)+c$，解决不限制区间个数的问题，求得其最优解为 $f_c(n)$．随着实数 $c$ 递增，相应的最优区间的数目单调递减，故而可以通过二分的方法找到恰使得最优区间个数等于 $m$ 的参数 $c$，则原题最优解为 $f(n,m) = f_c(n)-cm$．这里的实数 $c$ 可以看作区间个数限制的 Lagrange 乘子．该算法的实现有很多细节，可以参考 [WQS 二分](./WQS二分.md) 页面．这一算法的时间复杂度为 $O(n\log n\log C)$，这里 $C$ 为某一常数．
