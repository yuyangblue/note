...
-   [Dev-C++](../工具/editor/DevC++.md)
-   [Code::Blocks](../工具/editor/CodeBlocks.md)

### 使用 代码编辑器 + 编译器 + 调试器

可以参考 [VS Code](../工具/editor/VSCode.md) 页面中内容．Visual Studio Code 官方网站上有文档解释如何进行 C++ 的配置．一般而言 VS Code 搭配插件使用更方便，见 [VS Code 的官方网站](https://code.visualstudio.com/)．
...
> [!note]- 有关 std:: 前缀
> 有关 `std::` 这个前缀的问题，请见 [这节](基础.md#cin-与-cout) 底下的注释「什么是 std？」．
> 
...
2.  更多的输入输出语法参考 [这节内容](基础.md#scanf-与-printf)，并试着了解 C++ 的格式化输出．
...
请直接参考 [运算](./运算符.md) 一文中内容．附录中也提供了运算符与数学函数语法对比表．
...
请参考 [string](csl/string容器.md) 页面中内容．
...
C++ 几乎可以在 **任何地方** 声明变量．请参考 [变量作用域](变量.md#变量作用域)．
...
C++ 标准库中 `<algorithm>` 有很多有用的函数比如快排、二分查找等，可以直接调用．请参考 [STL 算法](csl/algorithm头文件.md) 页面．

还有 STL 容器，比如数组、向量（可变大小的数组）、队列、栈等，附带很多函数．请参考 [STL 容器简介](csl/STL容器.md) 页面．
...
C/C++ 的指针是很灵活的东西，可以参考 [指针](指针.md) 页面．如果想要彻底理解指针，建议找本书或者参考手册仔细阅读．
