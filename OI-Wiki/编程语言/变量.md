...
> 
> [!note] C 语言的布尔类型
> 另请参阅 [C++ 与其他常用语言的区别 - bool](./C++与其他语言.md#bool)．
> 
...
[^note16]: C++20 前规定有符号整数至少要覆盖 [反码](../数学/位运算.md#整数与位序列) 的表示范围（即 $-2^{x-1}+1\sim 2^{x-1}-1$），但实际上绝大多数实现中均采用 [补码](../数学/位运算.md#整数与位序列) 实现；C++20 起进一步规定有符号整数必须使用补码实现．详见 [Range of values - cppreference](https://en.cppreference.com/w/cpp/language/types.html#Range_of_values)．
