...
Lambda 表达式因数学中的 $\lambda$ 演算得名，直接对应于其中的 lambda 抽象．编译器在编译时会根据语法生成一个匿名的 [**函数对象**](./动态内存.md#函数对象)，以捕获的变量作为其成员，参数和函数体用于实现 `operator()` 重载．
...
lambda 以 capture 子句开头，它指定哪些变量被捕获，捕获列表可为空，或指定捕获方式：有 `&` 符号前缀的变量通过 [引用](./引用.md) 访问，没有该前缀的变量通过值访问．
...
1.  显式指定 $dfs$ 的类型，可以使用 `std::function` 替代．
    ...
    ??? warning "不建议使用 [`std::function`](./动态内存.md#stdfunction) 实现的递归"
