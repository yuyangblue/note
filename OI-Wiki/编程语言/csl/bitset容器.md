...
令原可重集为 $A$，其约数构成的可重集为 $A'$，我们要求 $A$ 中 $x$ 的个数，用 [莫比乌斯反演](../../数学/number-theory/莫比乌斯反演.md) 推一推：
...
由于 `bitset` 快速的连续读写效率，使得它非常适合用于与 [埃氏筛](../../数学/number-theory/筛法.md#埃拉托斯特尼筛法) 结合打质数表．
...
`bitset` 与树分块结合可以解决一类求树上多条路径信息并的问题，详见 [数据结构/树分块](../../数据结构/树分块.md)．

### 与莫队结合

详见 [杂项/莫队配合 bitset](../../杂项/莫队配合bitset.md)．
