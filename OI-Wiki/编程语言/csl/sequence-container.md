...
`std::deque` 是 STL 提供的 [双端队列](../../数据结构/queue.md#双端队列) 数据结构．能够提供线性复杂度的插入和删除，以及常数复杂度的随机访问．
...
`std::list` 是 STL 提供的 [双向链表](../../数据结构/linked-list.md) 数据结构．能够提供线性复杂度的随机访问，以及常数复杂度的插入和删除．
...
`std::forward_list` 是 STL 提供的 [单向链表](../../数据结构/linked-list.md) 数据结构，相比于 `std::list` 减小了空间开销．
