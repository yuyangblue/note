...
`set` 是关联容器，含有键值类型对象的已排序集，搜索、移除和插入拥有对数复杂度．`set` 内部通常采用 [红黑树](../../数据结构/rbtree.md) 实现．[平衡二叉树](../../数据结构/bst.md) 的特性使得 `set` 非常适合处理需要同时兼顾查找、插入与删除的情况．
...
`map` 是有序键值对容器，它的元素的键是唯一的．搜索、移除和插入操作拥有对数复杂度．`map` 通常实现为 [红黑树](../../数据结构/rbtree.md)．
