...
STL [栈](../../数据结构/栈.md)(`std::stack`) 是一种后进先出 (Last In, First Out) 的容器适配器，仅支持查询或删除最后一个加入的元素（栈顶元素），不支持随机访问，且为了保证数据的严格有序性，不支持迭代器．
...
STL [队列](../../数据结构/队列.md)(`std::queue`) 是一种先进先出 (First In, First Out) 的容器适配器，仅支持查询或删除第一个加入的元素（队首元素），不支持随机访问，且为了保证数据的严格有序性，不支持迭代器．
...
优先队列 `std::priority_queue` 是一种 [堆](../../数据结构/堆.md)，一般为 [二叉堆](../../数据结构/二叉堆.md)．
