...
### 将左值转换为右值
> 
使用 `std::move` [转移](./值类别.md#stdmove) 对象的所有权．这通常见于局部变量之间，或参数与局部变量之间：
> 
```cpp
#include <iostream>
#include <string>
...
```
> 
但不是所有时候都需要这么做，比如 [函数返回值优化](./值类别.md#常见误区)．
> 
### 右值延长临时量生命期
> 
从语义上，临时量可能会带来的额外的复制或移动，尽管多数情况下编译器能通过 [复制消除](./值类别.md#复制消除) 进行优化，但引用能强制编译器不进行这些多余操作，避免不确定性．
> 
## 参考内容
> 
