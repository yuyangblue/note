...
15 - 拼图是涉及 [启发式算法](../搜索/启发式搜索.md) 建模的经典问题．此问题的常见形式是 [曼哈顿距离](../计算几何/距离.md#曼哈顿距离) 和错位方块的数量计算，二者都是可接受启发（英文：admissible heuristic），即它们永远不会高估剩余的移动次数，这确保了某些搜索算法（例如 [A \* 算法](../搜索/A星算法.md)）的最优性．
...
N-Puzzle 支持常见的基于图的搜索算法，如广度优先搜索和深度优先搜索，同样我们也可以用 [A \* 搜索](../搜索/A星算法.md) 算法寻找最优解．启发式函数 $h(n)$ 可以是
