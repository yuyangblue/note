...
> 
随后 Lewis、Goodman 及 Miller 于 1969 提出，取 $A = 7^{5} = 16807$、$C = 0$、$M = 2^{31} - 1 = 2147483647$ 时，此算法可以达到该模数下近乎最大的周期 $2^{31} - 2$（因为 $16807$ 是该模数下的 [原根](../数学/number-theory/原根.md)）且具有相对不错的统计特征．
> 
