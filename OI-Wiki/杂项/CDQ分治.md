...
CDQ 分治是一种思想而不是具体的算法，与 [动态规划](../动态规划/索引.md) 类似．目前这个思想的拓展十分广泛，依原理与写法的不同，大致分为三类：
...
> 
> 为了方便枚举，我们把 $(l,mid)$ 和 $(mid+1,r)$ 中的点全部按照 $b$ 的值从小到大排个序．之后我们依次枚举每一个 $j$, 把所有 $b_{i} \leq b_{j}$ 的点 $i$ 全部插入到某种数据结构里（这里我们选择 [树状数组](../数据结构/树状数组.md)）．此时只要查询树状数组里有多少个点的 $c$ 值是小于等于 $c_{j}$ 的，我们就求出了对于这个点 $j$，有多少个 $i$ 可以合法匹配它了．
> 
> 当我们插入一个 $c$ 值等于 $x$ 的点时，我们就令树状数组的 $x$ 这个位置单点加一，而查询树状数组里有多少个点小于 $x$ 的操作实际上就是在求 [前缀和](../算法基础/前缀和.md)，只要我们事先对于所有的 $c$ 值做了 [离散化](../杂项/离散化.md)，我们的复杂度就是对的．
> 
...
> 
相关内容：[CDQ 分治优化 DP](../动态规划/opt/动态规划优化.md#cdq-分治优化-dp)
> 
...
> 
-   如果把询问 [离线](离线处理.md)，所有操作会按照时间自然地排成一个序列．
...
> 
> $pre$ 数组的具体变化可以使用 `std::set` 来进行处理．这个用 set 维护连续的区间的技巧也被称为 [old driver tree](./珂朵莉树.md)．
> 
> [!note]- 示例代码
> ```cpp
