...
???+ note "什么是 RMQ？"
    RMQ 是英文 Range Maximum/Minimum Query 的缩写，表示区间最大（最小）值．解决 RMQ 问题有很多种方法，可以参考 [RMQ 专题](../专题/rmq.md)．
...
ST 表基于 [倍增](../算法基础/binary-lifting.md) 思想，可以做到 $\Theta(n\log n)$ 预处理，$\Theta(1)$ 回答每个询问．但是不支持修改操作．
...
??? note "更严谨的证明"
    理解本段，可能需要具备 [时间复杂度](../算法基础/complexity.md) 的关于「势能分析法」的知识．
