...
并查集在经过修改后可以支持单个元素的删除、移动或维护树上的边权．使用动态开点线段树还可以实现 [可持久化并查集](./可持久化线段树.md#拓展基于主席树的可持久化并查集)．
...
同时使用路径压缩和启发式合并之后，并查集的每个操作平均时间仅为 $O(\alpha(n))$．其中，$\alpha$ 为阿克曼函数的反函数，增长极其缓慢．也就是说，并查集单次操作的平均运行时间可以认为是一个很小的常数．时间复杂度的证明在 [这个页面](./并查集复杂度.md) 中．
...
[最小生成树算法](../图论/最小生成树.md) 中的 Kruskal 和 [最近公共祖先](../图论/最近公共祖先.md) 中的 Tarjan 算法是基于并查集的算法．

相关专题见 [并查集应用](../专题/并查集应用.md)．
