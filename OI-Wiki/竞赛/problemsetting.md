...
使用时请注意 [LaTeX 公式的格式要求](../简介/format.md)．
...
请参考 [随机函数](../杂项/random.md) 页面．

需要特别提醒的是，在生成值域比随机函数返回值更大的数时，请 **不要** 使用 `rand() * rand()` 之类的写法，这样的写法生成的随机数非常不均匀．

另外，出题时推荐使用 [testlib](../工具/testlib/generator.md) 来造数据，可以保证在不同平台上同一个种子生成的随机数相同，并且种子会依据命令行参数自动生成．
...
真正均匀随机的方法是利用 [Prüfer 序列](../图论/prufer.md)，先生成一个随机 Prüfer 序列，再通过序列生成树．这样做的话，树的期望高度为 $O(\sqrt n)$．
...
让需要求最大公因数的两个数为 [斐波那契数列](../数学/combinatorics/fibonacci.md) 的相邻两项，可以让欧几里得算法达到最坏时间复杂度．
...
Polygon 是一个功能非常强大的多人合作出题平台，可以作为在任何网站（使用 package 功能导出到不支持 Polygon 的网站）多人合作出题的首选方案，单人出题（尤其是在不同设备上出题）时也是很不错的选择，使用方法参见 [Polygon 简介](../工具/polygon.md)．
