...
[莫比乌斯反演](./莫比乌斯反演.md) 就是利用 $\varepsilon=\mu \ast 1$ 对数论函数恒等式进行变形．
...
用抽象代数的语言说，这些代数性质说明，全体数论函数在（逐点）加法运算和 Dirichlet 卷积运算下构成 [交换环](../algebra/基础.md#环)，且它的全体可逆元就是那些在 $n=1$ 处取非零值的函数．这个环称为 **Dirichlet 环**（Dirichlet ring）．
...
用抽象代数的语言说，全体积性函数在 Dirichlet 卷积运算下构成 Dirichlet 环乘法群的 [子群](../algebra/群论.md#子群)．
...
???+ note "定理"
    ...
    3.  积性函数 $f$ 是完全积性函数，当且仅当 $f^{-1}=\mu f$，其中，$\mu$ 是 [莫比乌斯函数](./莫比乌斯反演.md#莫比乌斯函数)．
...
用抽象代数的语言说，如果 $\alpha$ 是完全积性函数，映射 $f\mapsto \alpha f$ 是 Dirichlet 环的 [自同态](../algebra/环论.md#理想)．
...
积性函数的特殊性同样反映在 Dirichlet 生成函数上．由于整数有 [唯一分解定理](./基础.md#算术基本定理)，积性函数 $f(n)$ 的生成函数 $F(s)$ 可写成如下形式：
...
以杜教筛一节的例题 [Luogu P3768 简单的数学题](../number-theory/双曲线法.md#例题) 为例，需要对 $f(n)=n^2\varphi(n)$ 构造满足上述条件的数论函数 $g(n)$．由于 $f$ 是积性函数，它的 Dirichlet 生成函数为
...
最后一步复杂度的估计与 [Eratosthenes 筛法](./筛法.md#埃拉托斯特尼筛法) 复杂度的证明一致．所以，本算法的时间复杂度为 $O(n\log\log n)$．
...
特别地，当积性函数 $g$ 是完全积性函数或其 Dirichlet 逆时，例如当 $g = 1$ 或 $g = \mu$ 时，那么算法可以进一步简化．此时，Dirichlet 卷积 $h = f\ast g$ 的计算可以采用常数更小的 [Dirichlet 前缀和/差分](./莫比乌斯反演.md#dirichlet-前缀和) 算法，但是算法时间复杂度仍为 $O(n\log\log n)$．

### 结果为积性函数的情形

最后，考虑 $h$ 是积性函数的情形．特别地，当 $f,g$ 都是积性函数时，$h=f \ast g$ 就是积性函数．要计算 $h$，只需要确定它在素数幂处的取值，就可以通过 [线性筛](./筛法.md#线性筛法) 在 $O(n)$ 时间内计算．而对于素数幂 $p^e$ 处的取值 $h(p^e)$ 直接暴力计算即可：
