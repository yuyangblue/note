...
2.  对于第 $i$ 个方程：
    1.  计算 $m_i=\frac{n}{n_i}$；
    2.  计算 $m_i$ 在模 $n_i$ 意义下的 [逆元](./反演变换.md)  $m_i^{-1}$；
...
令 $r_{ij}$ 为 $p_i$ 在模 $p_j$ 意义下的 [逆](./反演变换.md)：
...
而计算一个组合数对较小的质数取模后的结果，可以利用 [卢卡斯定理](./Lucas定理.md)．
...
由 [裴蜀定理](./裴蜀定理.md)，当 $a_2-a_1$ 不能被 $\gcd(m_1,m_2)$ 整除时，无解；

其他情况下，可以通过 [扩展欧几里得算法](./最大公约数.md) 解出来一组可行解 $(p, q)$；
