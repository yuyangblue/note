...
模运算下的高次剩余，可以认为是在讨论模意义下开高次方的可行性．它是 [二次剩余](./二次剩余.md) 的推广．
...
### 性质
> 
类似二次剩余，可以讨论 $k$ 次剩余的判定、个数以及 $k$ 次剩余类的个数问题．和其他 [同余方程](./同余方程.md) 问题一样，可以通过 [中国剩余定理](./中国剩余定理.md) 将它们转化为素数幂模的情形．根据原根的有无，这进一步区分为奇素数幂模和模数为 $2$ 的幂次的情形．
> 
奇数幂模的情形较为简单．事实上，对于所有原根存在的情形，都有如下结论：
> 
> [!note]- 定理
> 设整数 $k\geq 2$，整数 $a$ 和正整数 $m$ 互素．设模 $m$ 的原根存在，且 $g$ 是模 $m$ 的一个原根．记 $d=\gcd(k,\varphi(m))$ 且 $d'=\dfrac{\varphi(m)}{d}$，其中，$\varphi(m)$ 是 [欧拉函数](./欧拉函数.md)．那么，有：
> 
...
> $$
> 
> 其中，$\operatorname{ind}_g a$ 是离散对数．根据 [阶的性质](./原根.md#幂的循环结构) 和 $\delta_m(g)=\varphi(m)$，这等价于同余方程
> 
> $$
> ky \equiv \operatorname{ind}_g a \pmod{\varphi(m)}.
> $$
> 
> 这是关于 $y$ 的 [线性同余方程](./线性同余方程.md)．应用该页面对其解结构的分析，就可以知道方程有解当且仅当 $d\mid\operatorname{ind}_g a$，且通解形式为
> 
> $$
> y = y_0 + id' \pmod{\varphi(m)},~0\le y_0 < d',~i=0,1,\cdots,d-1.
> $$
> 
> 由此，就几乎可以得到本定理的全部内容；唯一需要额外说明的是判别式 $a^{d'} \equiv 1 \pmod m$．由 [阶的性质 3](./原根.md#幂的循环结构) 可知
> 
> $$
...
> $$
> 
> 又已知方程有解当且仅当 $d\mid \operatorname{ind}_g a$，亦即 $\delta_m(a)\mid d'$．由 [阶的性质 2](./原根.md#幂的循环结构) 可知，这就等价于该判别式．
> 
模数为 $2$ 的幂次的情形较为特殊．为处理这种情形，需要用到关于模 $2^e$ 既约剩余系结构的一个 [结论](./原根.md#递推公式)：所有奇数 $a$ 都唯一地同余于某个 $(-1)^s5^r\bmod 2^e$ 形式的整数，其中，$s\in\{0,1\}$ 且 $0\le r < 2^{e-2}$．借助这一结果，可以得到如下结论：
> 
> [!note]- 定理
...
> 
> [!note]- 证明
> 因为 $a\perp m$，所以 $x\perp m$．因为 $x$ 和 $a$ 都是奇数，由前述结论可知，可以设 $a\equiv (-1)^s5^r\pmod{2^e}$ 且 $x=(-1)^z5^{y}\pmod{2^e}$．因为表示是唯一的，所以同余方程 $x^k\equiv a\pmod{2^e}$ 等价于 [线性同余方程](./线性同余方程.md) 组
> 
> $$
> \begin{aligned}
> kz &\equiv s \pmod{2},\\
...
>     将两者结合就得到原方程的通解．
> 
这就完全解决了不同模数下 $k$ 次剩余的判定问题．二次剩余中的 Legendre 记号和二次互反律等内容也可以推广到高次剩余的情形，但这并不容易，需要用到 [分圆域](../algebra/域论.md#分圆域) 等概念．在代数数论中，二次互反律最终可以推广到 [Artin 互反律](https://en.wikipedia.org/wiki/Artin_reciprocity)．
> 
## 单位根
> 
作为 $k$ 次方根的特殊情形，本节讨论 $k$ 次（本原）单位根的概念．它可以看作是复数域 $\mathbf C$ 中 $k$ 次 [单位根](../复数.md#单位根) 的概念在模 $m$ 既约剩余系 $\mathbf Z_m^*$ 中的对应．当模数 $m$ 合适时，用模 $m$ 的 $k$ 次本原单位根代替复数根 $\omega_k$ 可以加速计算．
> 
类似于复数域的情形，有如下定义：
> 
...
> 
比较 [原根的定义](./原根.md#原根) 可知，原根 $g$ 就是模 $m$ 的 $\varphi(m)$ 次本原单位根，其中，$\varphi(m)$ 是 [欧拉函数](./欧拉函数.md)．
> 
当模 $m$ 的 $k$ 次本原单位根存在时，它的代数性质和 $k$ 次本原单位复根 $\omega_k$ 一致，可以代替 $\omega_k$ 进行各种计算．例如，将它应用于 [快速傅里叶变换](../poly/快速傅里叶变换.md) 中，就得到有限域[^fnnt]上的 [快速数论变换](../poly/快速数论变换.md)．
> 
### 性质
> 
...
> 
> [!note]- 性质
> 对于模数 $m$，设 $\lambda(m)$ 为它的 [Carmichael 函数](./原根.md#carmichael-函数)，有：
> 
> 1.  所有与 $m$ 互素的整数 $a$ 都是模 $m$ 的 $\delta_m(a)$ 次本原单位根，其中，$\delta_m(a)$ 是 $a$ 模 $m$ 的 [阶](./原根.md#阶)．
...
> 
> 直接验证定义可知，只要 $k\mid k'$，就可以从 $a^k\equiv 1\pmod m$ 推出 $a^{k'}\equiv 1\pmod m$，这就是性质 2．根据 [阶的性质](./原根.md#幂的循环结构) 可知
> 
> $$
...
> 
> 几乎重复 [原根判定定理](./原根.md#原根判定定理) 的证明，就可以得到性质 6．这一判别方法实际上在验证 $\delta_m(a)=k$．
> 
...
> $$
> 
> 根据对线性同余方程的 [分析](./线性同余方程.md) 可知，这一条件就等价于方程
> 
> $$
...
> 
这一定理说明，原根存在时，全体 $k$ 次单位根呈现 [循环群](../algebra/群论.md#循环群) 的结构，而 $k$ 次本原单位根则是该循环群的生成元．稍后将会看到，Tonelli–Shanks 算法正是利用这一点，加速了开方运算中离散对数部分的计算．
> 
## 模意义下开方
> 
最后，本文讨论 $k$ 次方根的求法．对于 $k=2$ 的情形，有 [很多高效算法](./二次剩余.md#模意义下开平方) 可以用于模意义下开平方运算．但是，对于一般的 $k$，并没有已知的多项式时间算法．本节将介绍两种常见算法，分别可以在 $O(m^{1/2})$ 和 $O(m^{1/4+\varepsilon})$ 时间内求出一个 $k$ 次方根．利用中国剩余定理总是可以将问题转换为素数幂模的情形，因此，本节主要讨论素数幂模情形的解法．
> 
### 朴素算法
> 
...
> $$
> 
> 其中，$\operatorname{ind}_g a$ 可以通过 [BSGS 算法](./离散对数.md#大步小步算法) 求出，而 [线性同余方程](./线性同余方程.md) 的全体解容易求出．由此，就可以得到 $a$ 的全部 $k$ 次方根 $x\equiv g^y\pmod m$．
> 
...
> 
将用于模意义下开平方的 [Tonelli–Shanks 算法](./二次剩余.md#tonellishanks-算法) 做适当推广，就可以解决素数幂模下开方运算．一种较为直接的推广方式是 Adleman–Manders–Miller 算法[^amm]，但是它的复杂度仍然不够优秀[^amm-comp]．本节介绍由 sugarknri、Min\_25、37zigen 等人提出的改良 Tonelli–Shanks 算法．它可以在 $O(m^{1/4+\varepsilon})$ 时间内求出一个 $k$ 次方根．
> 
...
$$
> 
最左侧同余号利用了 [欧拉定理](./费马小定理.md#欧拉定理) 和如下同余关系：（$c\in\mathbf Z$）
> 
$$
...
> 
进而，可以考虑算法总的时间复杂度．计算 $\varphi(m),d,\ell$ 的时间复杂度均为 $O(\log m)$．紧接着需要做素因数分解 $d=\prod_p{p^e}$，这一步利用 [Pollard Rho 算法](./Pollard-Rho算法.md#pollard-rho-算法) 可以在 $O(m^{1/4})$ 时间内完成．最后，依次求 $p^e$ 次方根的总时间复杂度为
> 
$$
