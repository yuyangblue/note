...
最大公约数与最小公倍数的性质见 [数论基础](./基础.md#最大公约数与最小公倍数)．
...
> 
事实上，假如我们试着用欧几里得算法去求 [斐波那契数列](../combinatorics/斐波那契.md) 相邻两项的最大公约数，会让该算法达到最坏复杂度．
...
> [!note] 关于 countr_zero
> 1.  gcc 有 [内建函数](../位运算.md#gcc-内建函数) `__builtin_ctz`（32 位）或 `__builtin_ctzll`（64 位）可替换上述代码的 `countr_zero`；
