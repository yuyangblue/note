...
> [!info] 关于连分数相关的算法实现
> 本文会提供一系列的连分数的算法实现，其中部分算法可能无法保证计算中间过程所涉及的整数都在 32 位或 64 位整型变量的取值范围内．对于这种情形，请参考相应的 Python 的实现，或将 C++ 实现中的整型变量替换为 [高精度整数类](../高精度.md)．为突出重点，本文行文过程中的部分代码可能会调用前文实现过的函数而不再重复给出实现．
> 
## 连分数
...
此时的计算过程实际上是对 $p$ 和 $q$ 做 [辗转相除法](./最大公约数.md#欧几里得算法)．这也说明，对于有理数 $x=\dfrac{p}{q}$，连分数表示的长度是 $O(\log\min\{p, q\})$ 的．计算有理数 $\dfrac{p}{q}$ 的复杂度也是 $O(\log\min\{p, q\})$ 的．
...
> $$
> 
> 其中，$\{F_k\}$ 是 [斐波那契数列](../combinatorics/斐波那契.md)．根据它的通项公式可知，
> 
> $$
...
> 
> [!note]- 解答
> 虽然这个问题通常是用 [扩展欧几里得算法](./裴蜀定理.md#两个变量的情形) 解决的，但是同样可以通过连分数求解．
> 
...
> -   直线 $y=\xi x$ 的方向向量是 $\vec\xi=(1,\xi)$．利用 [叉积](../linear-algebra/矩阵乘积.md#二维向量的情形) $(x_1,y_1)\times(x_2,y_2)=x_1y_2-x_2y_1$ 的概念，可以通过 $\vec\xi\times\vec\nu=p-q\xi$ 的正负判断点在直线上方还是下方．因而，在直线上方的点就对应着大于等于 $\xi$ 的分数，在直线下方的点就对应着小于等于 $\xi$ 的分数．叉积的绝对值 $|\vec\xi\times\vec\nu|$ 正比于点 $\vec\nu$ 与直线 $y=\xi x$ 的距离
> 
>     $$
...
>     $$
> 
>     根据 [Pick 定理](../../计算几何/Pick定理.md)，这意味着如果设 $I$ 和 $B$ 分别为三角形内部和边界上的整点个数，则
> 
>     $$
...
主条目：[Stern–Brocot 树与 Farey 序列](./Stern-Brocot树.md)

Stern–Brocot 树是存储了所有位于 $[0,\infty]$ 之间的分数的 [二叉搜索树](../../数据结构/二叉搜索树.md)．有限连分数实际上编码了 Stern–Brocot 树上从根到某个分数所在位置的路径．也就是说，有理数 $x$ 的连分数表示 $[a_0,a_1,\ldots,a_{n-1},1]$ 意味着从树根 $\dfrac{1}{1}$ 开始，需要先向右子节点移动 $a_0$ 次，再向左子节点移动 $a_1$ 次，交替方向移动，直到向某个方向移动了 $a_{n-1}$ 次为止．应当注意，此处只能使用末尾为 $1$ 的连分数表示．
...
> 
> [!note]- 解答
> 首先将两个连分数表示都转化成末尾是 $1$ 的形式．不妨设题目所给的已经是这样形式的连分数，即 $\alpha_n=\beta_m=1$．因为偶数位置（下标从 $0$ 开始）是向右移动的步数，奇数位置是向左移动的步数，所以，$\alpha<\beta$ 当且仅当按照 [字典序](../../字符串/基础.md#字典序) 比较时，有
> 
> $$
...
> 
> [!note]- 解答
> 因为 Stern–Brocot 树既是 $[0,\infty]$ 中的分数的二叉搜索树，又是二元组 $(q,p)$ 的 [笛卡尔树](../../数据结构/笛卡尔树.md)，所以题意几乎可以转化为求 Stern–Brocot 树上两个点的 LCA（最近公共祖先）．但是，LCA 只能处理闭区间内的情形，LCA 可能是端点本身．为了避免额外的讨论，可以首先构造出 $\dfrac{p_0}{q_0}+\varepsilon$ 和 $\dfrac{p_1}{q_1}-\varepsilon$，再计算 LCA．在已经通过连分数计算出根到节点的路径的情况下，LCA 只要取最长的公共路径即可．
> 
...
> 则它们有如下性质：[^pgl2]
> 
> 1.  分式线性变换的复合 $L_1\circ L_2$ 和逆变换 $L_1^{-1}$ 仍然是分式线性变换，即全体分式线性变换构成 [群](../algebra/基础.md#群)；
...
与循环连分数密切相关的概念是 [（实）二次无理数](./二次互反律.md)（quadratic irrational），即整系数二次方程的无理数解．所有的二次无理数都可以表示成
...
> 
> [!note]- 解答
> 首先利用上述算法解出 $\sqrt{x}$ 的周期，将循环节表示成分式线性变换，就可以用 [快速幂](../快速幂.md) 获得 $x_k$ 的值．当然，对于没有进入循环节和不足一个循环节的部分，需要单独处理．
> 
> **C++**
>     ```cpp
...
> 
二次无理数 $\sqrt{D}$ 的连分数展开主要应用在 [Pell 方程](./佩尔方程.md) 的求解中．
...
> 
> [!note]- 解答
> 这个和等于集合 $\{(x,y):1\le x\le N,1\le y\le\mathrm{e}x\}$ 中的整点个数．在构建完直线 $y=\mathrm{e}x$ 下的整点的凸包后，可以使用 [Pick 定理](../../计算几何/Pick定理.md) 计算整点个数．时间复杂度为 $O(\log N)$．
> 
...
> 
> [!note]- 解答
> 这是到目前为止最为复杂的题目．它可以通过 [类欧几里得算法](./欧几里得算法.md) 计算．此处给出基于连分数的算法，时间复杂度是 $O(\log\min\{A,B\})$．
> 
...
> 
> [!note]- 解答
> 根据 [中国剩余定理](./中国剩余定理.md)，询问对多个素数取模后的结果，相当于询问对这些素数的乘积取模的结果．因此，本题可以看作是询问分数对足够大的模数 $m$ 取模后的结果，要求确定分数的分子和分母．
> 
