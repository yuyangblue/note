...
关于公约数、公倍数、最大公约数与最小公倍数，四个名词的定义，见 [最大公约数](./最大公约数.md)．
...
互素的性质与最大公约数理论：裴蜀定理（Bézout's identity）．见 [裴蜀定理](./裴蜀定理.md)．

### 辗转相除法

辗转相除法是一种算法，也称 Euclid 算法．见 [最大公约数](./最大公约数.md)．
...
??? tip "Tip"
    这个定义的动机可以从 [素理想](../algebra/环论.md#素理想) 中找到．
...
由 [商群](../algebra/群论.md#商群) 的定义可知 $\mathbf{Z}_m=\mathbf{Z}/m\mathbf{Z}$，所以有时我们也会用 $\mathbf{Z}/m\mathbf{Z}$ 表示 $\mathbf{Z}_m$．

由 [抽屉原理](../combinatorics/抽屉原理.md) 可知：
...
???+ note "既约同余类"
    ...
    我们把模 $m$ 既约剩余类的个数记作 $\varphi(m)$，称其为 [Euler 函数](./欧拉函数.md)．
...
这些乃至更一般的类似形式求和式的推导可以参考 [类欧几里得算法](./欧几里得算法.md) 页面．

取整函数的更多性质以及应用可以参考如下页面：

-   取模运算：$n\bmod m = n - \left\lfloor\dfrac{n}{m}\right\rfloor m$．它可以用于 [优化整数取模运算](./模运算.md#相关算法)．
-   利用 Gauss 引理证明 [二次互反律](./二次剩余.md#二次互反律)．
-   [数论分块](./数论分块.md)，尤其是它的性质证明部分．
-   计算阶乘中素数因子幂次的 [Legendre 公式](./阶乘.md#legendre-公式)．
-   [Beatty 数列](../game-theory/公平组合游戏.md#wythoff-游戏)、Rayleigh 定理以及 Wythoff 博弈．
