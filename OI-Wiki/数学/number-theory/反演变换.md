...
这相当于说，$b$ 是线性同余方程 $ax\equiv 1\pmod m$ 的解．根据 [线性同余方程](./线性同余方程.md) 的性质可知，当且仅当 $\gcd(a,m)=1$，即 $a,m$ 互素时，逆元 $a^{-1}\bmod m$ 存在，且在模 $m$ 的意义下是唯一的．
...
求解逆元，就相当于求解线性同余方程．因此，可以使用 [扩展欧几里得算法](./最大公约数.md#扩展欧几里得算法) 在 $O(\log\min\{a,m\})$ 时间内求解逆元．同时，由于逆元对应的线性方程比较特殊，可以适当地简化相应的步骤．
...
这一方法主要适用于模数是素数 $p$ 的情形．此时，由 [费马小定理](./费马小定理.md#费马小定理) 可知对于任意 $a\perp p$ 都有

$$
a\cdot a^{p-2} = a^{p-1} \equiv 1 \pmod p.
$$

根据逆元的唯一性可知，逆元 $a^{-1}\bmod p$ 就等于 $a^{p-2}\bmod p$，因此可以直接使用 [快速幂](../快速幂.md) 在 $O(\log p)$ 时间内计算：
...
当然，理论上，这一方法可以利用 [欧拉定理](./费马小定理.md#欧拉定理) 推广到一般的模数 $m$ 的情形，即利用 $a^{\varphi(m)-1}\bmod m$ 计算逆元．但是，单次求解 [欧拉函数](./欧拉函数.md) $\varphi(m)$ 并不容易，因此该算法在一般情况下效率不高．
