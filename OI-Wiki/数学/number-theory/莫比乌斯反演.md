...
前置知识：[数论分块](./数论分块.md)、[狄利克雷卷积](./狄利克雷卷积.md#dirichlet-卷积)
...
??? note "证明"
    令 $n=\prod_{i=1}^kp_i^{e_i}$，设 $n' = \prod_{i=1}^kp_i$．根据 [二项式定理](../combinatorics/组合数.md#二项式定理)，有
...
如果需要对单个 $n$ 计算莫比乌斯函数 $\mu(n)$ 的值，可以利用它的 [质因数分解](./Pollard-Rho算法.md)．例如，在 $n$ 不太大时，可以在 $O(\sqrt{n})$ 时间内求出 $\mu(n)$ 的值．
...
如果需要对前 $n$ 个正整数预处理出 $\mu(n)$ 的值，可以利用它是积性函数，通过 [线性筛](./筛法.md#筛法求莫比乌斯函数) 在 $O(n)$ 时间内计算．
...
???+ example "例子"
    1.  [欧拉函数](./欧拉函数.md) $\varphi(n)$ 满足关系式 $n = \sum_{d\mid n}\varphi(d)$，亦即 $\mathrm{id}=1*\varphi$．对它进行反演，就得到 $\varphi = \mu * \mathrm{id}$，亦即
...
其次，莫比乌斯反演并不仅限于加法，它实际上对于任何 [Abel 群](../algebra/基础.md#群) 中的运算都成立．例如，它有如下的乘法形式：
...
从 Dirichlet 卷积的角度看，莫比乌斯反演只是利用了「莫比乌斯函数是常值函数的 Dirichlet 逆」这一点．容易想象，类似莫比乌斯反演的关系对于一般的 [Dirichlet 逆](./狄利克雷卷积.md#dirichlet-卷积) 同样成立．
...
??? note "证明"
    由 Dirichlet 卷积的 [性质](./狄利克雷卷积.md#性质) 可知，对于完全积性函数 $t(n)$，它的 Dirichlet 逆就是 $\mu(n)t(n)$．
...
接下来，讨论 Dirichlet 前缀和的计算．如果将每一个素数都看作一个维度，这就是一种高维前缀和．回忆高维前缀和的 [逐维前缀和算法](../../算法基础/前缀和.md#逐维前缀和)：逐个遍历所有的维度，并将每个位置的值都累加到该位置在该维度上的后继位置．对于数论函数，这相当于说，从小到大遍历所有素数 $p$，并将 $n$ 处的函数值累加到 $np$ 处．这和 [Eratosthenes 筛法](./筛法.md#埃拉托斯特尼筛法) 的遍历顺序是一致的．因此，这一算法可以在 $O(n\log\log n)$ 时间内计算出长度为 $n$ 的数列的 Dirichlet 前缀和．类似地，利用逐维差分就可以在相同时间复杂度内求出数列的 Dirichlet 差分．
...
接下来的两道例题展示了枚举公因数的处理方法，并利用 [筛法](./筛法.md#一般的积性函数) 计算一般积性函数的值．
...
??? note "解答"
    ...
    其中，$G(n)=\dfrac{1}{2}n(n+1)$ 就是等差数列求和，最后一个等号利用了 [下取整函数](./基础.md#取整函数) 的性质．对称地，对于另一个和式可以类似计算．代回前文表达式，就有
...
??? note "解答一"
    ...
    进一步地提取因数（即令 $i=di'$，$j=dj'$），并应用 [下取整函数](./基础.md#取整函数) 的性质，就得到：
    ...
    值得注意的是，涉及乘积的计算时，往往需要用到 [欧拉定理](./费马小定理.md)，因此指数部分取模用到的模数与题目所给的模数并不相同．
