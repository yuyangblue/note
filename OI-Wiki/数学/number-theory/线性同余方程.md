...
本文接下来介绍了两种求解线性同余方程的思路，分别利用了逆元和不定方程．对于一般的情形，逆元和不定方程的求解都需要用到 [扩展欧几里得算法](./最大公约数.md#扩展欧几里得算法)，因此，这两种思路其实是一致的．

## 用逆元求解

首先，考虑 $a$ 和 $n$ 互素的情形，即 $\gcd(a,n)=1$ 的情形．此时，可以计算 $a$ 的 [逆元](./反演变换.md)  $a^{-1}$，并将方程两边同乘以 $a^{-1}$，这就得到方程的唯一解：
...
线性同余方程等价于关于 $x,y$ 的 [二元一次不定方程](./裴蜀定理.md#两个变量的情形)：
