...
### 性质
> 
接下来讨论 Stern–Brocot 树的性质．简而言之，Stern–Brocot 树是包含所有正的最简有理分数的 [二叉搜索树](../../数据结构/二叉搜索树.md)，它也是分子和分母的 [堆](../../数据结构/二叉堆.md)，也是分母和分子构成的二元组的 [笛卡尔树](../../数据结构/笛卡尔树.md)．如果考虑前文的三元组构造中的左右端点形成的区间，则 Stern–Brocot 树也可以看作是 $[0,\infty]$ 上的 [线段树](../../数据结构/线段树.md)．这些陈述可以通过如下的三条基本性质导出：
> 
#### 单调性
> 
...
$$
> 
根节点处是单位矩阵，这显然成立．向下移动时，乘以的矩阵 $L$ 和 $R$ 的行列式都是 $1$，根据 [行列式](../linear-algebra/行列式.md) 的性质可知，在下一层同样成立
> 
$$
\det\begin{pmatrix}b & b+d \\ a & a+c \end{pmatrix} = \det\begin{pmatrix}b+d & d \\ a+c & c\end{pmatrix} = 1.
$$
> 
对于归纳起点 $\dfrac{0}{1}$ 和 $\dfrac{1}{0}$ 这也是显然的．由此，根据 [裴蜀定理](./裴蜀定理.md)，必然可知每个分数的分子和分母都是互素的，即所有分数都是最简分数．
> 
#### 完全性
> 
最后，需要说明 Stern–Brocot 树包括了所有正的最简分数．因为前两条性质已经说明 Stern–Brocot 树是二叉搜索树，而任何正的最简分数 $\dfrac{p}{q}$ 必然位于 $\dfrac{0}{1}$ 和 $\dfrac{1}{0}$ 之间，根据二叉搜索树上做搜索的方法，二叉搜索树上没有 $\dfrac{p}{q}$ 的唯一可能性就是搜索过程无限长．下面将证明，对于任意正有理数 $\dfrac{p}{q}$，无限长的搜索过程是不可能的．
> 
设搜索过程中得到的 $\dfrac{p}{q}$ 的下界为 $\dfrac{a}{b}$，上界为 $\dfrac{c}{d}$．下界初始值为 $\dfrac{0}{1}$，上界初始值为 $\dfrac{1}{0}$，随后上下界通过 $\dfrac{a+c}{b+d}$ 和 $\dfrac{p}{q}$ 的比较结果逐步调整．若搜索过程无限长，则每一步都有
> 
$$
...
$$
> 
利用前面已经说明的等式 $bc-ad=1$ 可将上式化简为
> 
$$
p+q \ge a+b+c+d.
$$
> 
然而，每次搜索更深入一层的时候，等式右侧都严格地增加，而左侧保持不变，因此上述条件无法在无限步内成立．因此，搜索过程会在有限步内停止．
> 
### 查找分数
> 
...
> 
如果对两个树分别做 [广度优先搜索](../../图论/广度优先搜索.md) 并依次对节点编号，根节点编号为 $1$，那么编号为 $v$ 的节点的左右子节点分别是 $2v$ 和 $2v+1$．从编号的二进制表示来看，除去起始的 $1$，从高位至低位每个 $1$ 均表示向右子节点移动，每个 $0$ 均表示向左子节点移动．Calkin–Wilf 树上，连分数 $[t_0,t_1,\cdots,t_n,1]$ 表示的有理数所在节点的编号是
> 
$$
...
$$
> 
此处的 $\varphi(n)$ 是 [欧拉函数](./欧拉函数.md)．该式利用 [线性筛](./筛法.md#筛法求欧拉函数) 可以 $O(n)$ 求出，而利用 [杜教筛](./双曲线法.md#杜教筛) 可以将复杂度降到 $O(n^{2/3})$．
> 
...
$$
> 
要得到右式，应用了 [莫比乌斯反演](./莫比乌斯反演.md)．线性筛和枚举因子结合，可以做到 $O(n)$ 预处理和 $O(n\log n)$ 询问；杜教筛和 [类欧几里得算法](./欧几里得算法.md) 结合，可以做到 $O(n^{2/3})$ 预处理和 $O(\sqrt n\log n)$ 询问．
> 
...
$$
> 
反过来，这也是两个最简真分数成为 Farey 邻项的充分条件．现在说明这一点．不妨设 $\dfrac{a}{b}$ 是两个分数中分母较大的，那么两个分数都会出现在 $F_b$ 中．设 $\dfrac{e}{f}$ 是序列 $F_b$ 中排在 $\dfrac{a}{b}$ 右侧的一项，按照已经说明的必要性可知，$be-af=1$．但是，[线性同余方程](./线性同余方程.md)  $bx-ay=1$ 只有一组 $y\le b$ 的正整数解，故而必然有 $(e,f)=(c,d)$．
> 
...
> 
要计算当前分数 $\dfrac{p}{q}$ 的其它 Farey 邻项，只需要利用 [扩展欧几里得算法](./裴蜀定理.md#两个变量的情形) 求出所有符合条件的解即可．
> 
### 递推关系
> 
