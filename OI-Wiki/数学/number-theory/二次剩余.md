...
二次剩余可以认为是在讨论求模意义下 **开平方** 运算的可行性．对于更高次方的开方可参见 [k 次剩余](./剩余系.md)．
...
> 
> [!note]- 证明
> 首先由 [Fermat 小定理](./费马小定理.md#费马小定理) 有 $a^{p-1}\equiv 1\pmod p$，故
> 
> $$
...
> $$
> 
> 由 [同余方程的定理 5](./同余方程.md#素数模同余方程) 可知，$a$ 是模 $p$ 的二次剩余当且仅当 $a^{(p-1)/2}\equiv 1\pmod p$. 进而 $a$ 是模 $p$ 的非二次剩余当且仅当 $a^{(p-1)/2}\equiv -1\pmod p$.
> 
...
> 
> 注意到 $\dfrac{p-1}{2}\mid (p-1)$，由 [同余方程的定理 6](./同余方程.md#素数模同余方程) 可知 $a^{\frac{p-1}{2}}\equiv 1\pmod p$ 有 $\dfrac{p-1}{2}$ 个解．所以模 $p$ 意义下二次剩余和二次非剩余均有 $\dfrac{p-1}{2}$ 个．
> 
## Legendre 符号
> 
为了方便接下来的讨论，我们引入如下记号：
...
> 4.  参见 [二次互反律](#二次互反律)
> 
基于如上性质，若对任意奇素数 $p$ 和 $q$，$\left(\dfrac{p}{q}\right)$ 的值均可计算，则我们就可以对任意合法情况计算 Legendre 符号的值．接下来我们有一个优美的定理，这个定理巧妙地在 $\left(\dfrac{p}{q}\right)$ 和 $\left(\dfrac{q}{p}\right)$ 之间建立起了联系，使得我们能用类似 [辗转相除法](./最大公约数.md#欧几里得算法) 的思路完成计算．
> 
### 二次互反律
> 
> [!note]- 二次互反律
...
## 模意义下开平方
> 
本节讨论模意义下开平方的算法．特别地，本节主要介绍素数模的情形．对于一般模数的情形，可以参考 [模意义下开高次方](./剩余系.md#模意义下开方) 的讨论．
> 
### 特殊情况时的算法
> 
...
> 
> [!note]- 计算方法
> 不熟悉 [多项式环](../algebra/环论.md#多项式环) 的读者，可以简单理解为该集合的元素都具有形式 $a_0+a_1x$ 且 $a_0,a_1\in\mathbf F_p$，且遵循如下运算法则：
> 
> $$
> \begin{aligned}
...
> 
> 关于该结构的更多内容，请参见 [多项式](../poly/简介.md) 和 [域论](../algebra/域论.md) 等页面．
> 
...
### Bostan–Mori 算法
> 
该算法基于 Cipolla 算法，我们将问题转换为 [常系数齐次线性递推](../poly/线性递推.md) 再应用 Bostan–Mori 算法．考虑另一种常见的 Cipolla 算法的描述：$b=x^{\left(p+1\right)/2}\bmod{\left(x^2-tx+a\right)}$ 为满足 $b^2\equiv a\pmod{p}$ 的一个解[^ref3]，其中 $x^2-tx+a\in \mathbf{F}_p\lbrack x\rbrack$ 为不可约多项式．系数 $t$ 的选取同样使用随机方法．证明过程略．参考 Bostan 和 Mori 论文[^ref4]中的算法我们可以发现问题可转化为求解形式幂级数的乘法逆元的某一项系数：
> 
$$
