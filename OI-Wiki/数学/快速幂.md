...
> -   当指数很大时，需利用 [扩展欧拉定理](./number-theory/费马小定理.md#扩展欧拉定理) 降幂后计算．
> 
### 计算斐波那契数

根据斐波那契数列的递推式 $F_n = F_{n-1} + F_{n-2}$，我们可以构建一个 $2\times 2$ 的矩阵来表示从 $F_i,F_{i+1}$ 到 $F_{i+1},F_{i+2}$ 的变换．于是在计算这个矩阵的 $n$ 次幂的时候，我们使用快速幂的思想，可以在 $\Theta(\log n)$ 的时间内计算出结果．对于更多的细节参见 [斐波那契数列](./combinatorics/斐波那契.md)，矩阵快速幂的实现参见 [矩阵加速递推](../数学/linear-algebra/矩阵.md#矩阵加速递推) 中的实现．
...
> 
简单地把这个置换取 $k$ 次幂，然后把它应用到序列上即可．时间复杂度为 $O(n \log k)$．对于更多的细节参见 [置换的复合](./置换.md#复合)．
...
> 
参考 [向量与矩阵](./linear-algebra/向量.md#向量与矩阵) 中的内容，每一种操作都可以用一个变换矩阵表示，一系列连续的变换可以用矩阵的乘积来表示．一个 Repeat 操作就相当于取一个矩阵的 $k$ 次幂．这样可以用 $O(m \log k)$ 的时间计算出整个变换序列最终形成的矩阵．最后将它应用到 $n$ 个点上，总复杂度 $O(n + m \log k)$．
...
> 
我们把该图的邻接矩阵 $M$ 取 $k$ 次幂，那么 $M_{i,j}$ 就表示从 $i$ 到 $j$ 长度为 $k$ 的路径的数目．该算法的复杂度是 $O(n^3 \log k)$．有关该算法的细节请参见 [矩阵](./linear-algebra/矩阵.md#定长路径统计) 页面．
...
但在实际使用中，此方法由于引入了更大的计算复杂度导致时间效率不优．实际编程中通常利用 [快速乘](./number-theory/模运算.md#快速乘) 来进行模数范围在 `long long` 时的乘法操作．

### 高精度快速幂

前置技能：[大整数乘法](./高精度.md#乘法)
...
当底数 $a$ 固定时，可以利用 [分块思想](../数据结构/分块思想.md)，用一定的时间预处理后用 $O(1)$ 的时间回答一次幂询问．这一算法也常称为光速幂．过程如下：
...
特别地，对于模意义下幂的计算，底数 $a$ 相同隐含着模数 $m$ 也要相同这一要求．由于 [扩展欧拉定理](./number-theory/费马小定理.md#扩展欧拉定理)，对于任意模数 $m$，预处理的指数范围上界为 $n = 2\varphi(m)$；对于素模数 $p$，预处理的范围上界为 $n = p - 1$．这两种情形预处理的复杂度都是 $O(\sqrt{m})$．
