...
另请参阅：[整数类型](../编程语言/var.md#整数类型)、[补数法](./numeral-sys/base.md#补数法)
...
在 C++ 中，我们用 `a << b` 表示左移，`a >> b` 表示右移，具体采用何种移位规则参见 [C++ 位操作符](../编程语言/op.md#位操作符)．
...
1.  高效地进行某些运算，代替其它低效的方式．参见 [编译优化 #强度削减](../编程语言/optimizations.md#强度削减-strength-reduction)．
2.  [表示集合](./binary-set.md)（常用于 [状压 DP](../动态规划/state.md)）．
3.  题目本来就要求进行位操作．
...
在 [状压 DP](../动态规划/state.md) 中，按照 popcount 递增的顺序枚举有时可以避免重复枚举状态．这是构造汉明权重递增的排列的一大作用．
...
如果需要操作的位序列非常长，可以使用 [`std::bitset`](../编程语言/csl/bitset.md)．
...
[^note1]: 一个数二进制表示从低往高的第一个 $1$ 连同后面的零，如 $(1010)_2$ 的 `lowbit` 是 $(0010)_2$，详见 [树状数组](../数据结构/fenwick.md)．
