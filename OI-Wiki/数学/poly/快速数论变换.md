...
**数论变换**（number-theoretic transform, NTT）是离散傅里叶变换（DFT）在数论基础上的实现；**快速数论变换**（fast number-theoretic transform, FNTT）是 [快速傅里叶变换](./快速傅里叶变换.md)（FFT）在数论基础上的实现．
...
学习数论变换需要前置知识：离散傅里叶变换、生成子群、[原根](../number-theory/原根.md)、离散对数．相关知识可以在对应页面中学习，此处不再赘述．

## 定义

### 数论变换

在数学中，NTT 是关于任意 [环](../algebra/基础.md#环) 上的离散傅立叶变换（DFT）．在有限域的情况下，通常称为数论变换（NTT）．
