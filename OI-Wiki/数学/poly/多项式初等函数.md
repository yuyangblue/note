...
参见 [Newton's Method](./牛顿法.md#newtons-method).
...
=== "多项式对数函数"
    首先，对于多项式 $f(x)$，若 $\ln{f(x)}$ 存在，则由其 [定义](./简介.md#复合)，其必须满足：
...
使用 [Newton's Method](./牛顿法.md#newtons-method) 即可在 $O(n\log{n})$ 的时间复杂度内解决多项式 $\exp$．
...
首先由 [Euler's formula](../复数.md#欧拉公式) $\left(\mathrm{e}^{\mathrm{i}x} = \cos{x} + \mathrm{i}\sin{x}\right)$ 可以得到 [三角函数的另一个表达式](https://en.wikipedia.org/wiki/Trigonometric_functions#Relationship_to_exponential_function_and_complex_numbers)：
