...
对应的 Venn 图和 [Hasse 图](./序理论.md#偏序集的可视化表示hasse-图)（以集合的包含关系 $\subseteq$ 为偏序，From [Wikipedia](https://en.wikipedia.org/wiki/File:Logical_connectives_Hasse_diagram.svg)）：
...
如果我们把 $\mathbf{B}$ 视作模 $2$ 的一个 [剩余类](./number-theory/基础.md#同余类与剩余系)，此时异或等价于模 $2$ 加法，与等价于模 $2$ 乘法，所以有时我们也用 $\mathbf{Z}_2$ 表示布尔域．
...
C++ 中的规定参见 [C++ 运算符优先级总表](../编程语言/运算符.md#c-运算符优先级总表)．
...
-   与、或均关于 $\mathbf{B}$ 构成 [交换幺半群](./algebra/基础.md#群)．即与运算和或运算均具有交换律、结合律和幺元（$x\land 1=x\lor 0=x$）．
-   异或、同或均关于 $\mathbf{B}$ 构成 [群](./algebra/基础.md#群)．即异或运算和同或运算均具有交换律、结合律、幺元（$x\oplus 0=x\odot 1=x$）和逆元（$x\oplus x=0$，$x\odot x=1$）．
