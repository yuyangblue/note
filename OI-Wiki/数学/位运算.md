...
另请参阅：[整数类型](../编程语言/变量.md#整数类型)、[补数法](./numeral-sys/进制.md#补数法)
...
位运算指的是对位序列逐位应用某些 [布尔函数](./布尔代数.md#布尔函数) 的运算．形式化地说，对布尔函数 $f:\mathbf{B}^k\to \mathbf{B}$，位运算即为形如
...
> 
在 C++ 中，我们用 `a << b` 表示左移，`a >> b` 表示右移，具体采用何种移位规则参见 [C++ 位操作符](../编程语言/运算符.md#位操作符)．
> 
我们可以用如下代码实现循环移位：
> 
> [!note]- 实现
...
位操作一般有三种作用：
> 
1.  高效地进行某些运算，代替其它低效的方式．参见 [编译优化 #强度削减](../编程语言/编译器优化.md#强度削减-strength-reduction)．
2.  [表示集合](./二进制集合.md)（常用于 [状压 DP](../动态规划/状态设计.md)）．
3.  题目本来就要求进行位操作．
> 
...
### 构造汉明权重递增的排列
> 
在 [状压 DP](../动态规划/状态设计.md) 中，按照 popcount 递增的顺序枚举有时可以避免重复枚举状态．这是构造汉明权重递增的排列的一大作用．
> 
...
### 更多位数
> 
如果需要操作的位序列非常长，可以使用 [`std::bitset`](../编程语言/csl/bitset容器.md)．
> 
## 题目推荐
> 
...
> 
[^note1]: 一个数二进制表示从低往高的第一个 $1$ 连同后面的零，如 $(1010)_2$ 的 `lowbit` 是 $(0010)_2$，详见 [树状数组](../数据结构/树状数组.md)．
> 