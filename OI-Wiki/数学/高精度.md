...
从 [进位制](./numeral-sys/进制.md) 的角度理解这一过程，我们通过在较大的进位制（上面每两位拆分一个数，可以认为是在 $100$ 进制下进行运算）下进行运算，从而达到减少参与运算的数字的位数，提升运算效率的目的．
...
普通的多项式乘法时间复杂度仍是 $O(n^2)$，但可以用多项式一节中的 [快速傅里叶变换](poly/快速傅里叶变换.md)、[快速数论变换](poly/快速数论变换.md) 等算法优化，优化后的时间复杂度是 $O(n\log n)$．
