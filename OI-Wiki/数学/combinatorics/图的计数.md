在组合数学中，图论计数（Graph Enumeration）是研究满足特定性质的图的计数问题的分支．[生成函数](../poly/简介.md)、[波利亚计数定理](./Pólya定理.md) 与 [符号化方法](../poly/符号化方法.md#%E9%9B%86%E5%90%88%E7%9A%84-cycle-%E6%9E%84%E9%80%A0) 和 [OEIS](https://oeis.org/) 是解决这类问题时最重要的数学工具．图论计数可分为有标号和无标号两大类问题，大多数情况下[^1]有标号版本的问题都比其对应的无标号问题更加简单，因此我们将先考察有标号问题的计数．

[^1]: 也许无标号二叉树是一个反例，在结构简单的情况下，对应的置换群是恒等群（Identity Group），此时有标号版本可以直接通过乘以 $n!$ 得到．

## 有标号树

即 Cayley 公式，参见 [Prüfer 序列](../../图论/Prüfer序列.md) 一文，我们也可以使用 [Kirchhoff 矩阵树定理](../../图论/矩阵树定理.md) 或 [生成函数](../poly/简介.md#生成函数) 和 [拉格朗日定理](https://codeforces.com/blog/entry/104184) 得到这一结果．
...
$$
> 
代换进上式得到 $CG = H$，使用 [多项式求逆](../poly/多项式初等函数.md#%E5%A4%9A%E9%A1%B9%E5%BC%8F%E6%B1%82%E9%80%86) 后再卷积解出 $C(x)$ 即可．
> 
#### 方法三：多项式 exp
> 
另一种做法是使用 [EGF 中多项式 exp 的组合意义](../poly/指数生成函数.md#egf-%E4%B8%AD%E5%A4%9A%E9%A1%B9%E5%BC%8F-exp-%E7%9A%84%E7%BB%84%E5%90%88%E6%84%8F%E4%B9%89)，我们设有标号连通图和简单图序列的 EGF 分别为 $C(x)$ 和 $G(x)$，那么它们将有下列关系：
> 
$$
\begin{align}
\exp(C(x)) &= G(x) \\
...
$$
> 
使用 [多项式 ln](../poly/多项式初等函数.md#多项式对数函数--指数函数) 解出 $C(x)$ 即可．
> 
## 有标号欧拉图、二分图
> 
...
$$
> 
[^2]: [粉兔的 blog](https://www.luogu.com.cn/blog/PinkRabbit/solution-sp4420) 告诉我们，这个序列也可以使用 [Chirp Z-Transform](../poly/Chirp-Z变换.md) 优化．
> 
...
## Riddell's Formula
> 
上述关于 EGF 的 exp 的用法，有时又被称作 Riddell's formula for labeled graphs，生成函数的 [欧拉变换](../poly/符号化方法.md#%E9%9B%86%E5%90%88%E7%9A%84-multiset-%E6%9E%84%E9%80%A0)，有时也被称为 Riddell's formula for unlabeled graphs，后者最早出现在欧拉对分拆数的研究中，除了解决图论计数问题之外，也在完全背包问题中出现．
> 
