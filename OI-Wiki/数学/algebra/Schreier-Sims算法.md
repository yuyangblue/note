...
前置知识：[抽象代数基本概念](./基础.md)、[群论](./群论.md)、[置换与排列](../置换.md)
...
类似于利用 [Gauss 消元法](../numerical/高斯消元.md) 构建出向量空间的一组 [线性基](../linear-algebra/线性基.md)，Schreier–Sims 算法的思路是找到有限置换群 $G$ 的一组「基」：
...
最简单的方法，当然是直接存储陪集代表系 $T$ 中的每一个元素 $t$．单个置换存储为 [单行记号](../置换.md#单行记号)，需要的空间恰为 $n$，所以存储这样的陪集代表系的空间复杂度是 $O(|T|n)$ 的．这样做的好处是访问单个陪集代表元的时间复杂度是 $O(1)$ 的，代价是初次计算这些陪集代表元的时间复杂度是 $O(|T|n)$ 的．
