...
-   数据结构中，[线段树](../../数据结构/线段树.md) 等结构可以维护幺半群的信息，而很多 DP 问题的递推关系可以抽象成这样的幺半群结构；
-   组合数学中，[Pólya 计数原理](../combinatorics/Pólya定理.md) 的严格表述和证明需要用到群论的相关概念．
...
???+ example "群的例子"
    -   **对称群**（symmetric group）：集合 $M$ 上的所有 [置换](../置换.md)，即自 $M$ 到 $M$ 自身的双射，就在映射的复合下构成群 $S_M$．单位元是恒等变换，逆元是逆映射（双射必然存在逆映射）．如果集合 $M$ 有限，大小为 $n$，也常记作 $S_n$，称作 $n$ 次对称群．
    -   空间对称群（symmetry group）：对于一个几何图形，能够使其与自身重合的变换全体也在映射的复合下构成群．这描述了该几何图形的空间对称性．具体例子可以参考 [常见空间对称群](../combinatorics/Pólya定理.md#常见空间对称群)．
    -   整数的加法群：整数集 $\mathbf Z$ 在加法 $+$ 运算下构成群 $(\mathbf Z,+)$．单位元是 $0$，逆元是相反数．
    -   整数模 $n$ 乘法群（multiplicative group of integers modulo $n$）：对于一个模数 $n$，所有与 $n$ 互质的整数对应的 [同余类](../number-theory/基础.md#同余类与剩余系)，在乘法运算下构成群 $((\mathbf Z/n\mathbf Z)^\times,\times)$．单位元是 $\bar 1$，逆元就是模 $n$ 的 [乘法逆元](../number-theory/反演变换.md)（对应的同余类），其存在性由 [裴蜀定理](../number-theory/裴蜀定理.md) 保证．具体结构分析参考 [整数模 $n$ 乘法群](./环论.md#应用整数同余类的乘法群)．
...
这些就是群论相关的基本定义．群论的更多内容，可以参考 [群论](./群论.md) 或相关书籍．
...
???+ example "环的例子"
    ...
    -   多项式环：对于一个环 $R$，可以在上面定义 [多项式环](./环论.md#多项式环)  $R[x]$．如果 $R$ 是整环，则该多项式环必然是整环．
...
当然，对于环的结构的讨论远不止这些，要了解更多内容，可以参考 [环论](./环论.md) 或相关书籍．
...
???+ example "域的例子"
    ...
    -   二次域（quadratic field）：它是在有理数域 $\mathbf Q$ 中添加了 $\sqrt d$ 而扩张成的，这里 $d\neq 0,1$ 且没有平方因子．相关内容可以参考 [二次域](../number-theory/二次互反律.md)．

域相较于环，拥有着非常简单的加法和乘法结构．所以，域本身的结构往往很简单．这使得域的研究和环的研究大不相同，通常会转而研究域的扩张，以及相应的 Galois 理论．在算法竞赛中，有时会需要在有理数域或者有限域的扩域上进行计算．域论的相关内容，可以参考 [域论](./域论.md) 或相关书籍．
...
???+ note "思路分析"
    这道题是动态 DP 模板，一种复杂度正确的代码实现需要用到 [全局平衡二叉树](../../数据结构/全局平衡二叉树.md)，具体样例代码也在对应页面．这里仅仅结合该题情景，分析建模的过程．
...
[^oi-wiki-not-wikipedia]: 因为 [OI Wiki 不是百科全书](../../简介/OIWiki不是什么.md#oi-wiki-不是百科全书)．
