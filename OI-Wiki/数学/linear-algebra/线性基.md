...
三维欧氏空间是特殊的 [线性空间](./向量空间.md)，三维欧氏空间的基向量在线性空间中就被推广为了线性基．
...
> 
> 如果不理解这条性质的正确性，可以跳转 [高斯消元](../numerical/高斯消元.md)．
> 
提供一组样例：
> 
```text
...
> $$
> 
> 根据 [线性映射的相关定理](./线性映射.md#线性映射的核空间与像空间)，有 $\dim H = \dim\pi(H)+\dim\ker\pi = \dim(U+W)+\dim(U\cap W)$．
> 
...
> 
对于需要多次查询区间异或最大值的情形，一种常见的做法是 [猫树](../../数据结构/Cat树.md) 配合线性基，时间复杂度为 $O(nm\log m+n^2q)$，其中，$n$ 是向量长度，$m$ 是序列长度，$q$ 是询问次数．另一种可行的做法是利用前缀线性基（或称时间戳线性基），可以将复杂度降低到 $O(n(m+q))$．
> 
