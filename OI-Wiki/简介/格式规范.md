...
> 
-   LaTeX 公式：
> -   不应与 [数学符号表](./符号约定.md) 相冲突．详见 [MATH-1.1](#MATH-1.1)．
> 
...
> 
-   <span id="SAVE-7">SAVE-7</span>：站内链接请去掉网站域名，并且使用相对路径链接对应 `.md` 文件．例如，在本页面（`intro/format`）中链接杂项简介（`misc`），应使用 `[杂项简介](../杂项/索引.md)`．可以在链接中添加 hash 来链接到某一节，例如 [`[Pull Request 信息格式规范](./如何参与.md#pull-request-信息格式规范)`](./如何参与.md#pull-request-信息格式规范)，hash 的值可以通过位于每个标题右侧的按钮或者位于网页右侧的目录中的链接得到．
> 
### 文档的合理性
> 
...
> 
除现有内容质量较低的情况外，建议尽量从 **补充** 的角度来做贡献，而非采取直接覆盖的方式．如果拿不准主意，可以参考 [关于本项目的交流方式](./关于OIWiki.md#交流方式) 一节，与 **OI Wiki** 项目组联系．
> 
### 文档的基本格式要求
> 
#### Remark-lint 的格式要求
...
> > 
    -   <span id="CONT-10.2.5">CONT-10.2.5</span>：不推荐对函数使用 `inline` 关键字，详见 [编译优化](../编程语言/编译器优化.md#inline---内联)．
> > 
...
> > > ```
> > > 
> > > 该代码给出了一个求 [最大公约数](../数学/number-theory/最大公约数.md) 的复杂实现，其中：
> > > 
...
> > > 
-   <span id="MATH-1.1">MATH-1.1</span>：您使用的符号不应与 [数学符号表](./符号约定.md) 规定的符号冲突．
> > > 
...
> > > 
    另外，由于 LaTeX 历史原因，空集的符号应为 `$\varnothing$` 而不是 `$\emptyset$`；其他的符号应参照 [数学符号表](./符号约定.md) 书写．
> > > 
...
> > > 
    为了提高代码复用率，您也可以将代码拆分成头文件，测试时在不同的测试代码里引用．如果正文中需要出现完整的测试代码作为例题的参考实现，那么正文中应该另外用 Snippet Sections 语法拼接成单文件代码，以便读者阅读．示例：[红黑树](https://github.com/OI-wiki/OI-wiki/blob/3b721e22ea60d59a2687a9b10555263de7bdc2f0/docs/ds/红黑树.md?plain=1#L218-L231)．
> > > 
关于例题代码：
> > > 
...
-   <span id="CODE-3.10">CODE-3.10</span>：由于 `long` 与 `unsigned long` 在某些测试环境下为 32 位，而在另一些测试环境下为 64 位，为确保各平台代码行为一致，故不推荐使用这两种类型．推荐使用 [定宽整数类型](../编程语言/变量.md#定宽整数类型)．
-   <span id="CODE-3.11">CODE-3.11</span>：不建议使用 `__gcd`、`__int128`、`__builtin_` 系列函数等非标准内容．如果您需要使用，则需确保您的代码能通过全平台测试，如 [此代码](https://github.com/OI-wiki/OI-wiki/blob/4af83d6db6017f4c36db6d4a7583bbc3f6257484/docs/ds/code/tree-decompose/tree-decompose_1.cpp#L24-L47) 提供了 libstdc++ 中 [std::bitset](../编程语言/csl/bitset容器.md) 特有成员函数 `_Find_first()` 的全平台实现．
> > > 
...
> > > 
[^ref3]: [我的公式为什么在目录里没有正常显示？好像双倍了](常见问题.md)
> > > 
