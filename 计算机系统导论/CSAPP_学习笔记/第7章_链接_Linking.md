...
> 依据：CMU 15-213 第 13 讲课件 
>
> `13-linking.pptx`
>
> （Bryant & O'Hallaron）+ 《深入理解计算机系统》原书第 3 版第七章（中文版）+ 课堂讲解。
...
> **课堂补充**：平时口头说 "编译" 常把四步全包含；严格说 "编译" 只是 cc1 那一步。
>
> **链接无法跳过**（每次构建都必须重新链接），但**编译可以跳过**：只改一个文件时，只需重编该文件 + 重新链接，其他文件不必重编 ——"是否要重编" 由文件时间戳判断（修改时间变了的文件才重编）。
...
> ⚠️ **本地链接器符号 ≠ 本地程序变量**：本地**非静态**变量（函数内普通局部变量）在运行时管理在**栈**中，`.symtab` 里**没有**条目，链接器不关心。本地**静态**变量（`static int x = 0;`）不在栈中，编译器在 `.data` 或 `.bss` 中分配空间，并在符号表中创建**唯一名字**的本地链接器符号：例如函数 f 和 g 中各有 `static int x`，编译器生成 `x.1`、`x.2`。
...
>
> **网课补充（CMU 15-213 L13）**：函数内 `static` 局部变量还有个语义要点 —— 它的值在**函数调用之间保留**（普通局部变量每次调用在栈上重新分配，static 的初值只在首次进入时初始化一次）。典型应用：伪随机数生成器把上一个随机数 / 种子存在 static 变量里，下次调用接着用。代价是这种函数**非线程安全**（多个线程共享同一份存储，讲线程时会再讨论）。

### 5.2 ELF 符号表条目结构（Elf64\_Symbol）
...
* **函数**必须强：代码只能有一份，链接器无法合并两个同名函数 —— 真要执行时该用哪一份？
...
**练习题 7.2**（书页 474）：在此题中，`REF(x.i)→DEF(x.k)` 表示链接器将把模块 i 中对符号 x 的任意引用与模块 k 中 x 的定义关联起来。对下面每个示例，用该表示法说明链接器将如何解析每个模块中对多重定义符号的引用。若有链接时错误（规则 1）写 "错误"；若链接器从定义中任意选择一个（规则 3）写 "未知"。
...
* A. 模块 1 `int main()`（强），模块 2 `int main;`（弱，未初始化全局变量）→ 规则 2 选强：


  * (a) REF(main.1) → DEF(main.1)；(b) REF(main.2) → DEF(main.1)

* B. 模块 1 `void main()`（强函数），模块 2 `int main = 1;`（强，已初始化）→ 规则 1：**错误**

* C. 模块 1 `int x;`（弱），模块 2 `double x = 1.0;`（强）→ 规则 2 选强：
...
* 只关心两种基本类型：
  ...
  * **R\_X86\_64\_PC32**：32 位 PC 相对地址引用（有效地址 = 指令中编码值 + PC 当前运行时值，PC 通常是**下一条指令**地址，如 call 的目标）。

  * **R\_X86\_64\_32**（PPT 中写作 R\_X86\_64\_32）：32 位**绝对地址**引用（直接使用编码值作为有效地址）。
...
* **网课补充（CMU 15-213 L13，中英精校字幕）**：现代 glibc 的 malloc 实现其实维护**两个堆区**—— 一个在高内存地址区域、**向下增长**，用于分配**大型对象**；另一个**向上增长**的堆用于**小型对象**。这样可以对大、小对象分别使用独立的分配算法，互不干扰。课本 / 课件画的是简化模型（一个堆），理解主流程按简化模型即可。
...
* `-fpic`：生成位置无关代码（见 12 节）；`-shared`：创建共享目标文件。
...
> **本节重点**：
> - **链接是什么**：把各种代码和数据片段收集组合成一个可执行文件的过程；可发生在编译时（静态链接）、加载时/运行时（动态链接）。
> - **符号解析**：每个符号引用绑定到唯一一个定义。强符号（函数、已初始化全局变量）vs 弱符号（未初始化全局变量），三条规则：强强同名 → 链接错误；强弱同名 → 选强；弱弱同名 → 任意选一个（`-fno-common` 可改为报错）。
> - **COMMON vs .bss**：未初始化全局变量 → COMMON；未初始化 static / 初始化为 0 的变量 → .bss；函数内局部变量不进符号表，static 局部变量进符号表且名字唯一化。
> - **静态库命令行顺序**：链接器从左到右扫描，**库必须放在引用它的目标文件之后**；库之间有依赖时被依赖的放后面，循环依赖重复列出。
> - **重定位**：PC 相对引用 `*refptr = ADDR(sym) + addend - refaddr`；`addend=-4` 是因为 PC 指向下一条指令。
> - **动态链接**：共享库用 `-fpic` 编译为 PIC；GOT + PLT 延迟绑定（首次调用解析，之后零额外开销）；运行时用 dlopen/dlsym 加载。
> - **库打桩**：编译时（本地头文件宏 + `-I.`）、链接时（`--wrap` + `__wrap_f`/`__real_f`）、运行时（`LD_PRELOAD` + `dlsym(RTLD_NEXT)`）。
> - **常见坑**：① 多重定义全局变量不报错（规则 2/3 静默覆盖）；② 静态库放错位置 → undefined reference；③ 弱符号类型不同会越界覆盖相邻变量（`-fno-common` 可查）。
