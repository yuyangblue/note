...
> **课堂补充**
>
> ：平时口头说 "编译" 常把四步全包含；严格说 "编译" 只是 cc1 那一步。
>
> **链接无法跳过**
>
> （每次构建都必须重新链接），但
...
> **编译可以跳过**
>
> ：只改一个文件时，只需重编该文件 + 重新链接，其他文件不必重编 ——"是否要重编" 由文件时间戳判断（修改时间变了的文件才重编）。
...
> `.exe`
>
>  结尾的（Windows 靠后缀关联程序，Linux 靠文件内容 / 权限判断）。
>
> `a.out`
>
>  名字来源于早期 Unix 的 "assembler output"。
...
> **本文件**
>
> 内可见，链接器不需要处理它们（它们不会与外部冲突）；用 static 保护模块私有名字是好习惯（类似 Java/C++ 的 private）。
> **网课补充（CMU 15-213 L13）**
>
> ：函数内 
>
...
> **函数调用之间保留**
>
> （普通局部变量每次调用在栈上重新分配，static 的初值只在首次进入时初始化一次）。典型应用：伪随机数生成器把上一个随机数 / 种子存在 static 变量里，下次调用接着用。代价是这种函数
>
> **非线程安全**
>
> （多个线程共享同一份存储，讲线程时会再讨论）。
...
> **课堂补充**
>
> ：规则 3 的问题在于随机选择一个弱符号，类型还可能不同（int 4 字节 vs double 8 字节），往里面写数据会越界覆盖相邻变量。多人协作时给全局变量 / 函数加
>
> **自己的名字前缀**
>
> （如 
...
> `callq 13`
>
>  中的 13 = 0x13 = 19，即下一条指令（add）的地址 —— 此时 call 目标未知，objdump 按 "跳到下一条指令" 占位显示。
...
> 为什么 addend = -4？因为 x86-64 中 call 指令的 PC 相对位移以
>
> **下一条指令**
>
> 为基准，而 refptr 指向的是
...
> **当前指令内偏移量字段的地址**
>
> （0x4004df），PC（0x4004e3）比 refaddr 大 4，所以要减 4 抵消。
